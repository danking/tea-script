Features
 - [ ] Gensym to handle non-JS allowed variable names
 - [ ] Gensym variable names align one to one to symbols of the same
       textual representation
 - [ ] BigNums
 - [ ] Bracket operator (array access, hash access)
 - [ ] a JOp struct that fathers all the primitive operations

Optimizations
 - [ ] ->csv* in translator.scm might benefit from not using format to append
       the recursive call

Failures
 - [ ] a define statement will have unnecessary tabbing before the first
       expression
