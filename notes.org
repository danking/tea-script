Features
 ...
 - [ ] prim ops should probably compile straight to JS
 - [ ] require/provide?
 - [X] a JOp struct that fathers all the primitive operations
 - [X] it would be super neat to have a single file that could run any
...
Failures
 ...
 - [ ] primops become identifiers which aren't bound in top-level JS
       this is due to sanitize-js.rkt
...
Goals for Friday
 - [X] support for tea `raise' (and JS `throw')
 - [X] complete support for the `letrec' form
 - [X] complete support for the `cond' form
